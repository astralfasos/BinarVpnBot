...
CHANNEL_USERNAME = "@BinarVPN"          # канал для подписки (должен начинаться с @)
CHANNEL_ID = CHANNEL_USERNAME            # можно использовать username
PROVIDER_TOKEN = ""                      # для Stars оставляем пустым

# Пакеты пополнения звездами (1 звезда Telegram = 1 единица внутреннего баланса)
DEPOSIT_PACKS = {
...
user_data = defaultdict(lambda: {
    'balance': 0,                         # звёзды на балансе
    'subscription_until': None,            # дата окончания подписки (ISO)
    'trial_used': False,                   # использована ли пробная подписка
...
def is_subscription_active(user_id: int) -> Tuple[bool, Optional[str]]:
    """Возвращает (активна ли, дата окончания строкой)"""
    until = user_data[user_id].get('subscription_until')
    if not until:
        return False, None
...
def activate_trial(user_id: int) -> bool:
    """Активирует пробную подписку на 3 дня, если ещё не использована."""
...
def generate_subscription_file(user_id: int) -> Optional[str]:
    """
    Генерирует подписку в формате Base64 для пользователя, если она активна.
    Возвращает готовую строку Base64.
    """
    ...
    # Создаём строку с информацией о сроке действия
    userinfo_line = f"#subscription-userinfo: upload=0; download=0; total=0; expire={expire_timestamp}"

    # Вставляем её в начало шаблона (можно и после других заголовков, но так проще)
...
async def start_command(message: Message):
    ...
    # Данные уже созданы через defaultdict
    welcome_text = (
...
async def trial_callback(callback: CallbackQuery):
    ...
    # Проверяем, не использована ли уже пробная подписка
    if user_data[user_id]['trial_used']:
    ...
    if not subscribed:
        ...
        if activate_trial(user_id):
            ...
            await callback.message.edit_text("❌ Не удалось активировать подписку. Попробуйте позже.")
...
async def profile_callback(callback: CallbackQuery):
    ...
    if active:
        sub_status = f"✅ Активна до {expire_str}"
    else:
...
async def connect_device_callback(callback: CallbackQuery):
    ...
    # Генерируем персональную подписку
    sub_base64 = generate_subscription_file(user_id)
    if sub_base64 is None:
        await callback.answer("Ошибка генерации подписки.", show_alert=True)
        return
    ...
    guide_text = (
        ...
        f"**📱 Для V2RayTun:**\n"
        f"1. Сохрани полученный файл в «Загрузки».\n"
        f"2. В приложении нажми «+» → «Импорт из файла» и выбери этот файл.\n"
        f"3. Подписка автоматически добавится.\n\n"
        f"**📱 Для Happ:**\n"
        f"1. Сохрани файл.\n"
        f"2. Открой приложение, перейди в раздел «Подписки».\n"
        f"3. Нажми «Добавить» → «Импорт из файла» и выбери файл.\n\n"
        f"Если функция «Импорт из файла» отсутствует, просто открой файл любым текстовым редактором, "
        f"скопируй весь текст и вставь его в приложение через функцию «Добавить подписку вручную».\n\n"
        f"⬇️ **Твой файл подписки:**"
    )

    # Создаём файл в памяти и отправляем
    file_data = io.BytesIO(sub_base64.encode())
    file_data.name = f"BinarVPN_{user_id}.txt"
